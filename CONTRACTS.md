...
proving them. The annotations are ordinary source: they compile to nothing
under a stock compiler, and `libzstd.a` builds unchanged and warning-free
without the contracts clang.
...
`assigns` is the write frame; `loop_invariant` and `decreases` are what replace
unrolling with induction.
